...
void imx6q_usbh1_pm_init(void)
{
	...
	writel_relaxed(BM_ANADIG_USB2_PLL_480_CTRL_ENABLE  \
			| BM_ANADIG_USB2_PLL_480_CTRL_POWER \
			| BM_ANADIG_USB2_PLL_480_CTRL_EN_USB_CLKS, \
			anatop_base + HW_ANADIG_USB2_PLL_480_CTRL_SET);

}

static void usbh1_internal_phy_clock_gate(bool on)
{
	void __iomem *phy_reg = IMX_IO_ADDRESS(MX6Q_USB_PHY2_BASE_ADDR);
	if (on) {
...
#define UCTRL_OVER_CUR_DIS      (1 << 7)        /* Disable OTG Overcurrent Detection */
	...
	tmp = readl_relaxed(otg_reg + UH1_CTRL);
	tmp |= UCTRL_OVER_CUR_POL;
	tmp |= UCTRL_OVER_CUR_DIS;
	writel_relaxed(tmp, otg_reg +UH1_CTRL);
...
static int imx6q_usbh1_init(struct platform_device *pdev, void __iomem *regs)
{
	int hub_reset, err;
	hub_reset = of_get_named_gpio(pdev->dev.of_node, "hub-reset-gpios", 0);
	if (hub_reset < 0) {
		printk(KERN_ERR "get gpio 'hub-reset-gpios' fails\n");
		return -EINVAL;
	}
	err = gpio_request_one(hub_reset, GPIOF_OUT_INIT_LOW, "hub-reset");
	if (err) {
		printk(KERN_ERR "request gpio 'hub-reset' fails\n");
		return -EINVAL;
	}
...
struct mxc_usbh_platform_data imx6q_usbh1_pdata = {
